...
export default function SubCategoryTab({ list }: SubCategoryTabProps) {
  ...
  return (
    ...
    >
      ...
      {/* 스크롤 영역 */}
      <div ref={scrollContainerRef} className='relative z-10 w-full overflow-x-auto scrollbar-hide '>
        <div className={cn('flex items-center w-fit mx-auto min-w-full ', NAV_LAYOUT.h)}>
          <div className='flex-1' />
          <div className='flex items-center gap-6 md:gap-10 px-8 md:px-20 flex-shrink-0'>
            {list.map(item => (
              <button key={item.id}
                data-tab-id={item.id}
