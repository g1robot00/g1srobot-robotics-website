...
export default function HistorySection({ history, id }: HistorySectionProps) {
    ...
    const yearNav = useMemo(() => {
        ...
        // UI표시할 문자열로 변환
        return uniqueDecades.map((startYear, idx) => {
            if (idx === 0) return `현재 - ${startYear}`; 
    ...
    const groupedHistory = useMemo(() => {
        ...
        // 가장 최신 연대의 기준값 계산
        const maxYear = Math.max(...history.map(item => parseInt(item.year)));
        const latestDecadeValue = maxYear <= 2000 ? 2000 : Math.floor(maxYear/10) * 10;

        history.forEach(item => {
            const year = parseInt(item.year);
            const decadeValue = year <= 2000 ? 2000 : Math.floor(year / 10) * 10;

            const decadeKey = decadeValue === latestDecadeValue ? `현재 - ${decadeValue}` : (year <= 2000 ? "2009 - 2000" : `${decadeValue + 9} - ${decadeValue}`);

            if (!groups[decadeKey]) groups[decadeKey] = [];
